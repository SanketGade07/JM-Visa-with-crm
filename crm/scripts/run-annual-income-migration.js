#!/usr/bin/env node
/**
 * Applies supabase/annual_income_migration.sql to the Supabase Postgres database.
 *
 ...
 *
 * Usage: npm run migrate:annual-income
 */
const fs = require('fs');
...
async function verifyViaRest() {
  ...
  const res = await fetch(`${url}/rest/v1/leads?select=id,annualIncome&limit=1`, {
...
async function main() {
  ...
  if (!dbUrl) {
    console.error(
      'Missing database connection. Set DATABASE_URL in .env (Supabase → Settings → Database → URI)\n' +
        ...
        'Alternatively, paste supabase/annual_income_migration.sql into the Supabase SQL Editor.\n'
    );
    process.exit(1);
  }

  const sqlPath = path.join(__dirname, '..', 'supabase', 'annual_income_migration.sql');
  ...
  if (result.ok) {
    console.log('✓ Verified: leads.annualIncome is reachable via Supabase REST.\n');
  } else {
    console.warn(
