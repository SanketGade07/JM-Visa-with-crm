#!/usr/bin/env node
/**
 * Applies supabase/lead_drive_folder_migration.sql to the Supabase Postgres database.
 *
 ...
 *
 * Usage: npm run drive:migrate-lead-folder
 */
const fs = require('fs');
...
async function verifyViaRest() {
  ...
  const res = await fetch(`${url}/rest/v1/leads?select=id,driveFolderId&limit=1`, {
...
async function main() {
  console.log('\n=== driveFolderId column migration ===\n');
  ...
  if (!dbUrl) {
    console.error(
      'Missing database connection. Set DATABASE_URL in .env (Supabase → Settings → Database → URI)\n' +
        ...
        'Alternatively, paste supabase/lead_drive_folder_migration.sql into the Supabase SQL Editor.\n'
    );
    process.exit(1);
  }

  const sqlPath = path.join(__dirname, '..', 'supabase', 'lead_drive_folder_migration.sql');
  ...
  if (result.ok) {
    console.log('✓ Verified: leads.driveFolderId is reachable via Supabase REST.\n');
  } else {
    console.warn(
